...
if (process.env.NODE_ENV === 'mock') {
  ...
  // baseContextUrl = 'http://10.0.103.159/api/' //
  // baseContextUrl = 'http://11.91.152.190/api/'
  baseContextUrl = 'http://10.170.233.98/api/'
  // baseContextUrl = 'http://192.168.137.90:80/api/'
  // baseContextUrl = 'http://10.0.98.140/api/'
} else {
  baseContextUrl = '/api/'
}
...
axios.interceptors.request.use(
  config => {
    ...
    if (config.method === 'post') {
      ...
      const sso_service_ticket = utils.getParam('sso_service_ticket', '')
      if (sso_service_ticket) {
        newParams.sso_service_ticket = sso_service_ticket
      }
